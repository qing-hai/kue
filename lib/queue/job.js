...
exports.get = function(id, fn){
  ...
  client.hgetall('q:job:' + job.id, function(err, hash){
    ...
    try {
      if (hash.data) job.data = EJSON.parse(hash.data);
      fn(err, job);
    } catch (err) {
      fn(err);
    }
...
Job.prototype.set = function(key, val, fn){
  this.client.hset('q:job:' + this.id, key, val, fn || noop);
    //console.log(" set job value",key,val);
	
  if (key==="delay" && val){
  // delay
    var created_time= this.created_at||Date.now();
	this.client.hset('q:job:' + this.id, "delay_plus_created_at", this._delay+created_time);
  }  
...
Job.prototype.update = function(fn){
  ...
  // serialize json data
  try {
    json = EJSON.stringify(this.data);
  } catch (err) {
    return fn(err);
  }
