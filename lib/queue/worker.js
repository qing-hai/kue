...
Worker.prototype.error = function(err, job){
  // TODO: emit non "error"
  //fix "undefined" error messages when passing string errors
...
Worker.prototype.failed = function(job, err, fn){
  ...
  events.emit(job.id, 'failed',job.data?job.data.response:undefined);
...
Worker.prototype.process = function(job, fn){
  ...
  fn(job, function(err, pause){
	...
	events.emit(job.id, 'complete',job.data?job.data.response:undefined);
...
Worker.prototype.getJob = function(fn){
  ...
  client.blpop('q:' + self.type + ':jobs', 0, function(err) {
    ...
    if( !self.running ) {
        // rollback! so inactive jobs list remains consistent with inactive jobs set
        // and no job remains in inactive state
    ...
    self.zpop('q:jobs:' + self.type + ':inactive', function(err, id){
	   if (err){
		...
		if (!id ) {
		  process.nextTick(function(){ self.start(fn); });
		}
		else{
			Job.get(id, fn);
		}
